...
*** Transaction creation
    ...
    :END:
    There X types of transactions created by the importer:
    - Overtime transactions
    - Worked day transactions
...
**** Overtime transactions
     ...
     :END:
     There is only one such transaction for any month in the input file. Any time exeeding the standard time provided as parameter to the =__init__()= method is considered overtime and is accumulated in a counter. Similarly, for any day where the worked period is below the standard time, the difference is substracted from the overtime counter for the current month.

     In case only half a day has been worked, the standard time is divided by 2.
