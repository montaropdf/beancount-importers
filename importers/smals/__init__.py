    ...
    def extract(self, file):
        ...
        for index, row in enumerate(csv.DictReader(open(file.name), dialect=csvDialect)): # , dialect=csvDialect
            ...
            if dtype2 == "PRE":
                ...
                overtime = decimal.Decimal((wk_time - self.standard_work_period).total_seconds() / 60)
                ...
                if dtype3 == "CAO":
                    desc = "Congé"
                    units_vacation = amount.Amount(decimal.Decimal('1'), self.commodity_vacation_day)
                    units_overtime = amount.Amount(decimal.Decimal(swp_minutes), self.commodity_overtime)
                    txn = data.Transaction(
